...
This class is a scheduler based on multi-processing. It is designed as an alternative [`keras.utils.Sequence`][keras-sequence]. The multi-processing codes are built on top of the [`multiprocessing`][pydoc-mp] module. It supports different workers and allows users to read datasets asynchronously and shuffle dataset randomly.
